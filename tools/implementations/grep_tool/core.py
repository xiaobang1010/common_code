...
if __name__ == "__main__":
    ...
    async def _test():
        ...
        # 测试 1：搜索 "import" 关键字
        inp = GrepInput(pattern="import", path="d:\\workspace\\code\\learning\\cc\\common_code", include="*.py")
        result = await tool.execute(inp, ToolUseContext())
        assert not result.is_error, f"期望 is_error=False，实际：{result.content}"
        assert "import" in result.content.lower(), f"期望包含 'import'，实际：{result.content[:200]}"
        print("[PASS] 搜索 import 关键字")

        # 测试 2：files_with_matches 模式
        inp2 = GrepInput(
            pattern="import",
            path="d:\\workspace\\code\\learning\\cc\\common_code",
            include="*.py",
            output_mode="files_with_matches",
        )
        result2 = await tool.execute(inp2, ToolUseContext())
        assert not result2.is_error
        assert "tool.py" in result2.content or "schema.py" in result2.content
        print("[PASS] files_with_matches 模式")
        ...
        assert "处匹配" in result3.content
        print("[PASS] count 模式")

        # 测试 4：无效正则
        inp4 = GrepInput(pattern="[invalid", path="d:\\workspace\\code\\learning\\cc\\common_code")
        result4 = await tool.execute(inp4, ToolUseContext())
        assert result4.is_error, "期望 is_error=True"
        print("[PASS] 无效正则")

        print("\nAll GrepTool tests passed!")
