...
if __name__ == "__main__":
    print("=== prompt_input/core.py 测试 ===\n")

    # 1. 测试 SlashCommandCompleter
    print("--- SlashCommandCompleter ---")
    ...
    doc2 = Document("/c", cursor_position=2)
    ...
    # 2. 测试 InputMode
    print("\n--- InputMode ---")
    print(f"  SINGLE_LINE: {InputMode.SINGLE_LINE.value}")
    print(f"  MULTI_LINE: {InputMode.MULTI_LINE.value}")

    # 3. 测试 PromptInput 基本属性
    print("\n--- PromptInput ---")
    single_input = PromptInput(multiline=False)
    print(f"  单行模式: {single_input.mode}")

    multi_input = PromptInput(multiline=True)
    print(f"  多行模式: {multi_input.mode}")

    # 4. 测试历史记录
    print("\n--- 历史记录 ---")
    test_input = PromptInput(multiline=False)
    test_input.add_to_history("hello world")
    test_input.add_to_history("/help")
    test_input.add_to_history("write a function")
    print(f"  历史记录项数: {len(test_input._history_items)}")
    for i, item in enumerate(test_input._history_items):
        print(f"    [{i}] {item}")

    # 5. 测试 set_history
    print("\n--- set_history ---")
    test_input.set_history(["item1", "item2", "item3"])
    print(f"  重设后历史记录项数: {len(test_input._history_items)}")
    for i, item in enumerate(test_input._history_items):
        print(f"    [{i}] {item}")

    # 6. 测试 SlashCommandLexer
    print("\n--- SlashCommandLexer ---")
    lexer = SlashCommandLexer()
    doc3 = Document("/help me please")
    get_line = lexer.lex_document(doc3)
    line0_fragments = get_line(0)
    print(f"  '/help me please' 的片段: {line0_fragments}")

    doc4 = Document("normal text here")
    get_line2 = lexer.lex_document(doc4)
    line0_fragments2 = get_line2(0)
    print(f"  'normal text here' 的片段: {line0_fragments2}")
