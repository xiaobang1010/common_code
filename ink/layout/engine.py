...
if __name__ == "__main__":
    ...
    # 测试 1: column 方向，两个文本
    print("\n--- 测试 1: column 方向 ---")
    root1 = Box(flex_direction="column", children=[
        Text("Hello"),
        Text("World"),
    ]).to_layout_node()
    engine.compute_layout(root1, 80, 24)
    print(f"Root: {root1.computed_layout}")
    for i, child in enumerate(root1.children):
        print(f"  Child[{i}] ({child.type}): {child.computed_layout}")

    # 验证
    assert root1.computed_layout.width == 80
    assert root1.computed_layout.height == 24
    assert root1.children[0].computed_layout.y == 0
    assert root1.children[1].computed_layout.y == 1

    # 测试 2: row 方向，两个文本
    print("\n--- 测试 2: row 方向 ---")
    root2 = Box(flex_direction="row", children=[
        Text("Hello"),
        Text("World"),
    ]).to_layout_node()
    engine.compute_layout(root2, 80, 24)
    print(f"Root: {root2.computed_layout}")
    for i, child in enumerate(root2.children):
        print(f"  Child[{i}] ({child.type}): {child.computed_layout}")

    assert root2.children[0].computed_layout.x == 0
    assert root2.children[1].computed_layout.x == 5  # "Hello" 宽度

    # 测试 3: flex_grow 分配
    print("\n--- 测试 3: flex_grow 分配 ---")
    root3 = Box(flex_direction="row", children=[
        Text("A"),
        Box(flex_grow=1),
        Text("B"),
    ]).to_layout_node()
    engine.compute_layout(root3, 80, 24)
    print(f"Root: {root3.computed_layout}")
    for i, child in enumerate(root3.children):
        print(f"  Child[{i}] ({child.type}): {child.computed_layout}")

    assert root3.children[0].computed_layout.width == 1  # "A"
    assert root3.children[2].computed_layout.width == 1  # "B"
    # 中间的 Box 应该占据剩余空间
    assert root3.children[1].computed_layout.width == 78

    # 测试 4: Spacer
    print("\n--- 测试 4: Spacer ---")
    root4 = Box(flex_direction="row", children=[
        Text("Left"),
        Spacer(),
        Text("Right"),
    ]).to_layout_node()
    engine.compute_layout(root4, 80, 24)
    print(f"Root: {root4.computed_layout}")
    for i, child in enumerate(root4.children):
        print(f"  Child[{i}] ({child.type}): {child.computed_layout}")

    assert root4.children[0].computed_layout.width == 4  # "Left"
    assert root4.children[2].computed_layout.width == 5  # "Right"
    assert root4.children[1].computed_layout.width == 71  # Spacer

    # 测试 5: padding
    print("\n--- 测试 5: padding ---")
    root5 = Box(flex_direction="column", padding=2, children=[
        Text("Padded"),
    ]).to_layout_node()
    engine.compute_layout(root5, 80, 24)
    print(f"Root: {root5.computed_layout}")
    for i, child in enumerate(root5.children):
        print(f"  Child[{i}] ({child.type}): {child.computed_layout}")

    assert root5.children[0].computed_layout.x == 2  # padding.left
    assert root5.children[0].computed_layout.y == 2  # padding.top

    # 测试 6: gap
    print("\n--- 测试 6: gap ---")
    root6 = Box(flex_direction="row", gap=2, children=[
        Text("A"),
        Text("B"),
        Text("C"),
    ]).to_layout_node()
    engine.compute_layout(root6, 80, 24)
    print(f"Root: {root6.computed_layout}")
    for i, child in enumerate(root6.children):
        print(f"  Child[{i}] ({child.type}): {child.computed_layout}")

    assert root6.children[0].computed_layout.x == 0
    assert root6.children[1].computed_layout.x == 3  # 1 + gap(2)
    assert root6.children[2].computed_layout.x == 6  # 3 + 1 + gap(2)

    # 测试 7: justify_content center
    print("\n--- 测试 7: justify_content center ---")
    root7 = Box(flex_direction="row", justify_content="center", children=[
        Text("Hi"),
    ]).to_layout_node()
    engine.compute_layout(root7, 80, 24)
    print(f"Root: {root7.computed_layout}")
    for i, child in enumerate(root7.children):
        print(f"  Child[{i}] ({child.type}): {child.computed_layout}")

    assert root7.children[0].computed_layout.x == 39  # (80 - 2) // 2

    # 测试 8: 嵌套 Box
    print("\n--- 测试 8: 嵌套 Box ---")
    root8 = Box(flex_direction="column", children=[
    ...
    engine.compute_layout(root8, 80, 24)
    print(f"Root: {root8.computed_layout}")
    for i, child in enumerate(root8.children):
        print(f"  Child[{i}] ({child.type}): {child.computed_layout}")
        for j, grandchild in enumerate(child.children):
            print(f"    Grandchild[{j}] ({grandchild.type}): {grandchild.computed_layout}")
