...
1. **配置初始化**：六层配置优先级逐层合并，环境变量最高，用户设置最低。
2. **会话搭建**：工作目录、Git 根、钩子快照、权限模式。
3. **延迟预取**：系统上下文、用户上下文、模型特性——首次用到才加载。

### 循环引擎

AI 自主工作的核心。每轮循环：压缩消息 → 注入上下文 → 流式调 LLM → 错误恢复 → 工具执行 → 进下一轮。工具参数一凑齐就异步开跑，不用等 LLM 把话说完。上下文超限时偷偷压缩重试，不打断你。

### 工具执行八步管线

解析 → 校验 → 前置钩子 → 权限决策 → 执行 → 后置钩子 → 封装结果。失败不退出，错误喂回模型让它自己决定下一步。

### 钩子

十种事件拦截器，让外部脚本介入 AI 的工作流程。危险命令拦截、敏感文件保护、变更审计日志——都是钩子干的。glob 匹配工具名，30 秒超时。
